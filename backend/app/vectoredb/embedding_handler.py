...
class EmbeddingHandler:
    ...
    def check_embeddings_exist(self, website_id: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                print(f" Filter failed, falling back to scroll: {filter_error}")
        ...
        for i, page in enumerate(website_data):
            try:
                ...
                if not content or len(content.strip()) < 50:
                    print(f" Skipping page {i+1}: No sufficient content (len={len(content) if content else 0})")
                    continue
    ...
    def search_similar_content(
        ...
        try:
            ...
            try:
                ...
                # If filter fails (index missing), fall back to scroll and manual filter
                print(f" Filtered search failed, falling back to manual filtering: {filter_error}")
    ...
    def search_uploaded_content(
        ...
        try:
            ...
            try:
                ...
                print(f" Filtered upload search failed, falling back to manual filtering")
    ...
    def delete_website_embeddings(self, website_id: str) -> bool:
        ...
        try:
            print(f" Attempting to delete embeddings for website: {website_id}")
            
            # Try with filter first (more efficient)
            try:
                ...
                # First, count how many points we're deleting
                count_result = self.qdrant_client.count(
                    collection_name=self.collection_name,
                ...
                if points_count == 0:
                    print(f" No embeddings found for website: {website_id}")
                    return True
                ...
                print(f" Filtered deletion failed, falling back to scroll-delete: {filter_error}")
                
                # Fallback: scroll through all points and delete one by one
                all_points = []
                next_page_offset = None
                ...
                # Filter points by website_id
                points_to_delete = []
                for point in all_points:
                    if point.payload and point.payload.get("website_id") == website_id:
                ...
                for i in range(0, len(points_to_delete), batch_size):
                    ...
                    print(f" Deleted batch {i//batch_size + 1}/{(len(points_to_delete)-1)//batch_size + 1}")
                
                print(f" Successfully deleted {len(points_to_delete)} embeddings for website: {website_id}")
                return True
