...
class ChatbotGenerator:
    ...
    def generate_script_url(self, website_id: str) -> str:
        ...
        # Create unique hash for the script
    ...
    def generate_script_file(self, website_id: str) -> str:
        """Generate JavaScript file for chatbot widget with Qdrant Cloud support"""
        
        # First check if website has embeddings in Qdrant
        if not self.check_website_has_embeddings(website_id):
            # Get website info to provide better error message
...
// Version: 5.1 (Fixed Enquiry Button UI)
...
(function() {{
    ...
    let chatbotState = {{
        ...
        isInitialized: false,
        lastUserQuestion: '',
        pendingEnquiryButton: false  // Track if enquiry button should be shown
    }};
    ...
    function createChatbot() {{
        ...
        chatWindow.innerHTML = `
            ...
            <!-- Persistent Enquiry Button Container (hidden by default) -->
            ...
            ">
                ...
                ">
                    📋 <span>Need Detailed Information? Fill Enquiry Form</span>
                </button>
            </div>
        `;
        ...
        styleSheet.textContent = `
            ...
            /* Persistent enquiry button styling */
            #persistent-enquiry-btn {{
                border: none;
            }}
            
            #persistent-enquiry-btn:hover {{
                background: linear-gradient(135deg, #EA580C, #C2410C);
                transform: translateY(-2px);
            }}
    ...
    function showContactForm() {{
        ...
        contactForm.innerHTML = `
            <h3 style="color: ${{config.accentColor}}; margin: 0 0 15px 0; font-size: 18px; display: flex; align-items: center; gap: 8px;">
                <span>📧</span> Enquiry Form
            </h3>
            <p style="margin-bottom: 15px; color: #4B5563; font-size: 14px;">Fill this form and our team will get back to you:</p>
        ...
        // Pre-fill with the last user question if available
    ...
    function shouldShowEnquiryButton(userQuestion, botResponse) {{
        // List of phrases that indicate the agent is suggesting an enquiry form
        ...
        // Check if bot response contains any enquiry phrase
        if (botResponse) {{
            const lowerResponse = botResponse.toLowerCase();
            for (const phrase of enquiryPhrases) {{
                if (lowerResponse.includes(phrase.toLowerCase())) {{
                    console.log('🔍 Enquiry phrase detected:', phrase);
                    return true;
                }}
            }}
        }}
        
        // Also check user question for enquiry-related keywords
        const enquiryKeywords = [
            ...
            'office', 'headquarters', 'meet', 'visit', 'enquiry', 'inquiry'
        ];
    ...
    function showPersistentEnquiryButton() {{
        ...
        if (container) {{
            ...
            console.log('🔔 Showing persistent enquiry button');
        }}
    }}
    ...
    function addMessage(type, content, forceEnquiryButton = false) {{
        ...
        // If enquiry button should be shown, also show persistent button
        ...
        if (type === 'user') {{
            ...
            // Add enquiry button INSIDE the message if needed
            if (shouldShowEnquiry) {{
                html += `
    ...
    async function handleContactFormSubmit() {{
        ...
        try {{
            ...
            // Hide persistent button after successful submission
    ...
    function setupEventListeners() {{
        ...
        // Add listener for persistent enquiry button
        if (persistentEnquiryBtn) {{
            persistentEnquiryBtn.addEventListener('click', showContactForm);
        }}
...
"""
        ...
        try:
            ...
            print(f"✅ Generated chatbot script with Qdrant Cloud support: {script_path}")
            return script_path
